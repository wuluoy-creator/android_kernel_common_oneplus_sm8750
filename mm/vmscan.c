...
restart:
	for (i = pte_index(start), addr = start; addr != end; i++, addr += PAGE_SIZE) {
		...
		if (pte_dirty(ptent) && should_mark_folio_dirty(folio))
				  ...
				  bool clear_pmd_young)
{
	...
	do {
		...
		if (pmd_dirty(pmd[i]) && should_mark_folio_dirty(folio))
			folio_mark_dirty(folio);
...
restart:
	...
	walk_pmd_range_locked(pud, -1, vma, args, bitmap, &first, clear_pmd_young);
		       ...
		       int hist)
{
	...
	int refs = ((flags & LRU_REFS_MASK) >> LRU_REFS_PGOFF) +
